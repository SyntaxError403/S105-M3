...
turtles-own [
  ...
  times-moved      ; number of times moved
]
...
to setup
  ...
  ask turtles with [ not happy? ]
    [set-inital-times-moved]
  reset-ticks
end

to set-inital-times-moved
  set times-moved 0 ; set the intial times moved
end
...
to find-new-spot
  if times-moved <= 3 [   ; set the times-moved resction to 3 times
  rt random-float 10
  fd random-float 10
...
to update-turtles
  ask turtles [
    ...
    set times-moved times-moved + 1 ; for very new move the turtle makes the times-moved value will increae by one
  ]
end
